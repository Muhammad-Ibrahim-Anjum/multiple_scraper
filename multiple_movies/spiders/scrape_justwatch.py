...
class ScrapeJustwatchSpider(scrapy.Spider):
    ...
    def search_result_links(self, response):
        urls = response.css(
            f'a.title-list-row__column-header span.header-year:contains("{YEAR}")')
    ...
    def parse_movie_page(self, response):
        ...
        year = response.xpath(
            '//div[@class="title-block"]//span[@class="text-muted"]/text()').get()
        year = year.replace('(', '').replace(')', '')

        if movie_url and "tv-series" in movie_url or "tv-show" in movie_url:
            h2_headings = response.css(
                'h2[class="detail-infos__subheading--label"]')
            for heading in h2_headings:
                heading_text = heading.css('*::text').get()
                if heading_text:
                    heading_text = heading_text.upper()
                    if 'SEASONS' in heading_text:
                        number_of_seasons = ''.join(
                            filter(str.isdigit, heading_text))
                        seasons_list = response.css(
                            f'div[itemamount="{number_of_seasons}"] div[class="horizontal-title-list__item"]')
    ...
    def parse_movie(self, response):
        ...
        imdb_img_tag = response.css(
            '.detail-infos__value .jw-scoring-listing__rating span img[alt="IMDB"]')
        ...
        for tag in detail_info_tags:
            ...
            if h3_tag and 'Genres' in h3_tag:
                genres = tag.css(
                    '[class="detail-infos__subheading"]+div::text').get()
            ...
            casts_tags = response.css(
                'div.title-credits__actor span.title-credit-name')
    ...
    def parse_seasons(self, response):
        ...
        title = response.xpath(
            '//div[@class="title-block"]//h1/a/text()').get()
        ...
        episodes = []
        episodes_list = response.css(
            '[class="episodes-item"] span[class="episodes-item__heading--title"]')
        ...
        for tag in detail_info_tags:
            ...
            if h3_tag:
                ...
                if 'Director' in h3_tag:
                    director = tag.css(
                        'div.detail-infos__value span::text').get()
            casts_tags = response.css(
                'div.title-credits__actor span.title-credit-name')
            for cast in casts_tags:
                actor = cast.css('::text').get()
